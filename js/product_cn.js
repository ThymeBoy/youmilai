...
// 瀑布流配置
METUI_FUN['product_list_page_met_28_1'] = {
    name:'product_list_page_met_28_1',
    metAnimOnScroll: function() {
        var list=METUI['product_list_page_met_28_1'].find('.met-product-list');
        // 产品列表页
        if(list.length){
            if($('#met-grid').length){
                setTimeout(function(){
    ...
    metshowhide:function(){
        var p=$(".met-product-list .product-li");
        p.each(function(){
            if($(this).find(".cover-image").length>1){
                $(this).on('mouseenter',function(){
                    $(this).find(".img").hide().siblings(".hover-img").show();
                });
                $(this).on('mouseleave ',function(){
                    $(this).find(".img").show().siblings(".hover-img").hide();
                });
            }
        });
    }
};
METUI['product_list_page_met_28_1'] = new metui(METUI_FUN['product_list_page_met_28_1']);
